...
namespace Afy
{
    public partial class AfyNuevoSuscriptor : Form
    {
        ...
        private void btnAceptar_Click(object sender, EventArgs e)
        {
            ...
            {
                    if(comprobarNumeros(txtCodigoSeguridad.Text) == false)
                    {
                        MessageBox.Show("El número CVV solo puede tener números.");
                    } else
                    {
                        if(comprobarNumeros(txtNombre.Text) == true)
                        {
                            MessageBox.Show("El nombre no puede contener números.");
                        } else
                        {
                            if(comprobarNumeros(txtApellido.Text) == true)
                            {
                                MessageBox.Show("El apellido no puede contener números.");
                            } else
                            {
                                if(comprobarNumeros(txtUsuario.Text) == true)
                                {
                                    MessageBox.Show("El usuario no puede contener solo números.");
                                } else
                                {
                                    if(comprobarNumeros(txtDireccion.Text) == true)
                                    {
                                        MessageBox.Show("La dirección no puede contener solo números.");
                                    } else
                                    {
                                        if(comprobarNumeros(txtLocalidad.Text) == true)
                                        {
                                            MessageBox.Show("La localidad no puede contener solo números.");
                                        } else
                                        {
                                            if(comprobarNumeros(txtTelefono.Text) == false)
                                            {
                                                MessageBox.Show("El telefono no puede tener letras.");
                                            } else
                                            {
